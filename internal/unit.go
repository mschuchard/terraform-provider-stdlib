	...
	for name, testCase := range testCases {
		...
		test.Run(name, func(test *testing.T) {
			...
			if !result.Result.Equal(expected.Result) {
				test.Errorf("expected type: %T, value: %v", expected.Result.Value(), expected.Result.Value())
				test.Errorf("actual type: %T, value: %v", result.Result.Value(), result.Result.Value())
			}
		})
	}
}
