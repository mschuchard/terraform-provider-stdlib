...
func (*cutFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
	...
	if len(inputString) < 1 {
		resp.Error = function.ConcatFuncErrors(resp.Error, function.NewArgumentFuncError(0, "input string parameter must be at least length 1"))
	}
	if len(separator) < 1 {
		resp.Error = function.ConcatFuncErrors(resp.Error, function.NewArgumentFuncError(1, "separator parameter must be at least length 1"))
	}
