...
func TestMinNumberFunction(test *testing.T) {
	test.Parallel()
	...
	standardTestCases := map[string]struct {
		...
		"list-length": {
			...
			expected: function.RunResponse{
				Error:  function.NewArgumentFuncError(0, "min_number: list parameter length must be at least 1"),
	...
	for name, testCase := range standardTestCases {
		test.Run(name, func(test *testing.T) {
			...
			slicefunc.NewMinNumberFunction().Run(context.Background(), testCase.request, &result)
